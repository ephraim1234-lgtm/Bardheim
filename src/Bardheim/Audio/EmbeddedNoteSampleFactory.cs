...
public static class EmbeddedNoteSampleFactory
{
    private const string ResourcePrefix = "Bardheim.Assets.Audio.";
    ...
    public static AudioClip? TryCreate(NoteDefinition note, ManualLogSource logger)
    {
        ...
        if (data is null)
        {
            logger.LogWarning($"Lyre note sample resource not found: {resourceName}");
            return null;
        }

        if (!TryReadPcm16Wav(data, out var samples, out var channels, out var sampleRate, out var error))
        {
            logger.LogWarning($"Could not load lyre note sample {note.Name}: {error}");
            return null;
        }
        ...
        logger.LogInfo($"Loaded lyre note sample {note.Name}.");
        return clip;
    }
