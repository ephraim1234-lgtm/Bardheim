...
public static class EmbeddedDrumSampleFactory
{
    private const string ResourcePrefix = "Bardheim.Assets.Audio.Drums.";

    public static AudioClip? TryCreate(string hitId, ManualLogSource logger)
    {
        var resourceName = $"{ResourcePrefix}{hitId}.wav";
        var data = EmbeddedAssetLoader.ReadResource(resourceName);
        if (data is null)
        {
            logger.LogWarning($"Drum hit sample resource not found: {resourceName}");
            return null;
        }

        if (!TryReadPcm16Wav(data, out var samples, out var channels, out var sampleRate, out var error))
        {
            logger.LogWarning($"Could not load drum hit sample {hitId}: {error}");
            return null;
        }
        ...
        logger.LogInfo($"Loaded drum hit sample {hitId}.");
        return clip;
    }
